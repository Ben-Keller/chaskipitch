...
export function CountryDetailPage({
  ...
  const { loading, error, data } = useAsyncData(
    async () => {
      const [country, boundaryGeo, territoryGeo, globalContent, mediaIndex, countrySignals] = await Promise.all([
        getCountryByIso(normalizedIso3),
        getCountryGeo(normalizedIso3, "boundary"),
        getCountryGeo(normalizedIso3, "territories"),
        getGlobalContent(),
        getMediaIndex(),
        getCountrySignalsByIso(normalizedIso3)
      ]);
  ...
  if (loading) {
    return <LoadingPanel label={`Loading ${normalizedIso3} story scene...`} />;
  }

  if (error || !data || !data.country) {
    return <ErrorPanel message="Country page not found or failed to load." />;
  }

  const { country, boundaryGeo, territoryGeo, globalContent, mediaIndex, countrySignals } = data;
    ...
    <div className="page-grid">
      <section className="country-scene">
        <div
          ...
          style={{
            backgroundImage: `url(${mediaPath(photos[0]?.file ?? "report-page-23.jpg")})`
          }}
          aria-hidden="true"
        />
        <div className="country-content">
          <div style={{ display: "grid", gap: "1rem" }}>
            <article className="float-card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.8rem" }}>
                ...
                <CountryBackButton onBack={() => onNavigateHome?.(country.iso3)} />
              </div>
                  ...
                  <details className="inline-disclosure">
                    ...
                    <ul style={{ marginTop: "0.4rem" }}>
                      {overflowSignalKpis.map((kpi) => (
                        <li key={kpi.id}>
                          <strong>{kpi.label}:</strong> {formatUnit(kpi.value, kpi.unit)} <SourcePill page={kpi.source_page} />
                        </li>
                      ))}
                    </ul>
                  </details>
          ...
          <div style={{ display: "grid", gap: "1rem", alignContent: "start" }}>
            <article className="float-card" id="country-geography">
              ...
              <CountryMapD3 boundaryGeo={boundaryGeo} territoryGeo={territoryGeo} />
              <p className="note" style={{ color: "rgba(243,232,210,0.7)", marginTop: "0.5rem" }}>
                Boundary source: geo/countries_manifest.json | Territory overlays: supplemental GeoJSON | <SourcePill page={sourcePage} />
              </p>
            </article>
      ...
      <section className="panel">
        ...
        <div className="controls-row">
          <button type="button" className="as-link" onClick={() => onNavigateHome?.(country.iso3)}>
            Home / Global Impact
          </button>
          <button type="button" className="as-link" onClick={onNavigateCountries}>
            Country Explorer
          </button>

...
function resolveSceneVideoSrc(srcPattern) {
  if (typeof srcPattern !== "string" || !srcPattern) {
    return "";
  }
  const replaced = srcPattern.replace(
    /\/(s\d+)\/seq\/([^/]+)\/frame_%04d\.(?:webp|png|jpg)$/i,
    "/mp4/$1/$2.mp4"
  );
...
function ScrubbedSceneVideo({ src, alt, zoom, localProgress, preload = "metadata" }) {
  ...
  useEffect(() => {
    ...
    const targetTime = clamp(localProgress, 0, 1) * duration;
    ...
    try {
      ...
      // Ignore transient seek issues while metadata is stabilizing.
    }
  }, [localProgress, resolvedSrc]);
...
export function StoryExperience({ story }) {
  ...
  const currentAudioOffsetRef = useRef(AUDIO_START_OFFSET_SECONDS);
  ...
  useEffect(() => {
    ...
    currentAudioSrcRef.current = targetSrc;
    currentAudioOffsetRef.current = soundtrackStartOffsetSeconds;
  }, [
    effectiveAudioVolume,
    hasEntered,
    isMuted,
  ...
  useEffect(() => {
    ...
    const cueAudio = resetAudioRef.current;
  ...
  return (
    ...
    >
      ...
      <audio ref={resetAudioRef} preload="auto" />
